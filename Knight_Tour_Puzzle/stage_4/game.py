def main():
    ...
    pos_moves = find_possible_moves(board, position)
    pos_moves_num = find_possible_move_num(board, pos_moves)
    board = mark_possible_moves(board, cell_size, pos_moves, pos_moves_num)
...
def set_starting_point(board: list, cell_size: int, coordinates: tuple) -> list:
    ...
    board[outer][inner] = 'X'.rjust(cell_size)
    return board
...
def mark_possible_moves(board: list, cell_size: int, moves: list, move_num: list) -> list:
    ...
    for move, number in zip(moves, move_num):
        ...
        board[outer][inner] = str(number).rjust(cell_size)
    return board
